...
const getUserProfile = async (req, res) => {
	...
	try {
		...
		if (mongoose.Types.ObjectId.isValid(query)) {
			...
			// query is username // name
			user = await User.findOne({ name: query }).select("-password").select("-updatedAt");
		}
...
const signupUser = async(req, res) => {
       try {
          const {name,collegeName,address,email,password} = req.body;
...
const updateUser = async (req, res) => {
  try {
    const {name,email,password,collegeName,address,bio} =req.body;
